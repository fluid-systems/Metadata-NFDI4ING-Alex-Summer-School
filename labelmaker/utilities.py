...
def register_font(font_name: str, font_path: str):
    """Register a TTF font with ReportLab.

    Args:
        font_name (str): Name to use for the font in ReportLab
...
def generate_pID_QR_code_label(
    label_file_path: [str, Path],
    qr_code_file_path: [str, Path],
    data_dict: dict,
):
    ...
    font_path = Path(__file__).parent / "font" / "JetBrainsMonoNerdFontMono-Regular.ttf"
    # Set font - use TTF if provided, otherwise fall back to default
    ...
    if font_path:
        ...
        if register_font(font_file_name, font_path):
            font_name = font_file_name
        else:
            print(f"[WARNING] Failed to register font {font_path}, using default font")
    ...
    qr_code__drawing.scale(
        scaling_factor, scaling_factor
    )  # scaling factor is equal for x and y since rectangular
    ...
    # Set the font family for the styles to use the registered Nerd Font
    normal_style.fontName = font_name
    pid_normal_style.fontName = font_name
    ...
    elif product_name == "acceleration":
        product_name = "{} 󰓅".format(product_name.upper())
    elif product_name == "raspberrypi":
        product_name = "{} ".format(product_name.upper())
...
def place_labels_on_DINA4_template(
    ...
    if not (sys.platform.startswith("linux") or sys.platform.startswith("win32")):
        ...
        raise Warning(
            f"You are running the FST Label Creator on the platform '{sys.platform}', that isn't tested yet. Please write the maintainers an email and share your findings with them to improve this software product. Thank You."
        )
    ...
    for item in resolved_path.glob("*.pdf"):
        subprocess.run(
            [
                "inkscape",
                "--export-type=svg",
                f"--export-filename={item.stem}.svg",
                f"{item.stem}.pdf",
            ]
        )
    os.chdir(str(cwd))
    ...
    if start_position_number > MAXIMUM_AMOUNT_OF_LABELS:
        ...
        raise Exception(
            f"The following label start position was declared: {start_position_number}\n"
    ...
    canvas = Canvas(
        f"{path_for_generated_files}/site{site_counter}.pdf", pagesize=DINA4_SITE_SIZE
    )
    ...
    for item in converted_svg_labels_list:
        ...
        # Calculate position.
        position_x = (
            measured_start_position[0]
            + (column_position_counter - 1) * measured_x_distance_per_step
        )
        position_y = (
            measured_start_position[1]
            + (row_position_counter - 1) * measured_y_distance_per_step
        )
        ...
        if row_position_counter > row_max_label_count:
            ...
            canvas = Canvas(
                f"{path_for_generated_files}/site{site_counter}.pdf",
                pagesize=DINA4_SITE_SIZE,
            )
...
def generate_text_QR_code_label(
    input_text: str,
    heading_text: str,
    label_size: tuple,
    file_name: str,
    ...
    canvas = Canvas(
        str(label_file_path.resolve()),
        pagesize=(label_size[0] * cm, label_size[1] * cm),
    )
    ...
    text_width = (
        label_width_in_points
        - scaled_drawing_width_in_points
        - (scaled_drawing_height_in_points * MEASURED_PADDING_FACTOR) / 2
    )
    text_height = label_width_in_points - (
        scaled_drawing_height_in_points * MEASURED_PADDING_FACTOR
    )
    w_heading, h_heading = heading_paragraph.wrap(text_width, text_height)
    heading_paragraph.drawOn(
        canvas,
        scaled_drawing_width_in_points,
        label_height_in_points
        - (scaled_drawing_height_in_points * MEASURED_PADDING_FACTOR)
        - h_heading,
    )
